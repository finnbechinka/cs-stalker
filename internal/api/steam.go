...
// Tries to fetch recently played games information and checks if cs is inculed.
//
// Returns total cs playtime in hours if cs is included, 0 otherwise (including on error).
func UserPlaytime(steamid64 string) (int32, error) {
	...
	for _, v := range autoGenerated.Response.Games {
		if v.Appid == 730 {
			minutes := int32(v.PlaytimeForever)
			hours := minutes / 60
			return hours, nil
		}
	}

	return 0, fmt.Errorf("UserPlaytime: Playtime not found")
}
